...
import org.opendaylight.yang.gen.v1.urn.opendaylight.soamcollector.rev161123.SoamNode;
...
public class TopologyChangeListener extends SoamDataTreeChangeListener<Node> {
    ...
    private final SoamProvider provider;

    public TopologyChangeListener(DataBroker dataBroker, SoamProvider provider) {
        super(dataBroker);
    ...
    public void add(DataTreeModification<Node> newDataObject) {
        ...
        SoamNode soamAugment = node.getAugmentation(SoamNode.class);
        if (soamAugment != null
                && soamAugment.getPollInterval() != null
                && soamAugment.getSnmpCommunity() != null) {
            provider.addNode(name,
                    new String(netconfAugment.getHost().getValue()),
                    soamAugment.getSnmpCommunity(),
                    soamAugment.getPollInterval());
        }
    }
